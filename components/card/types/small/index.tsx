...
const Small = ({ metadata, scope, text }: SmallProps) => {
  ...
  // * Shorthand for image source
  const imgSrc =
    metadata.twitter_img_src || metadata.twitter_img || metadata.og_img;

  // * Update the image error state on error
  const updateImgError = () => {
    setImgError(true);
  };

  // * If no valid image source on mount, set to fallback
  useEffect(() => {
    if (!imgSrc) updateImgError();
  }, []);
      ...
      <a className={styles.cardLink} href={metadata.og_url}>
        <div className={styles.summary}>
          <div className={styles.imageContainer}>
            {!(metadata.fallbackImg || imgError) ? (
              <img
                alt={
                  metadata.twitter_img_alt || metadata.og_img_alt || "Image."
                }
                className={styles.image}
                src={imgSrc}
                onError={updateImgError}
              />
            ) : (
